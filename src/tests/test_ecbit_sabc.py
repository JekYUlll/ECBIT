...
def test_station_adaptive_bias_correction_accepts_residual_features() -> None:
    layer = StationAdaptiveBiasCorrection(
        d_model=16,
        n_vars=3,
        n_station_features=STATION_FEATURE_DIM,
    ...
    z_era5 = torch.randn(2, 3, 16)
    station_features = torch.randn(2, STATION_FEATURE_DIM)
    residual_features = torch.randn(2, 3, RESIDUAL_FEATURE_DIM)

    y = layer(z_era5, station_features, residual_features)

    assert torch.allclose(y, z_era5, atol=1.0e-7)


def test_station_adaptive_bias_correction_requires_residual_features() -> None:
    layer = StationAdaptiveBiasCorrection(
        d_model=16,
        n_vars=3,
        n_station_features=STATION_FEATURE_DIM,
    ...
    with pytest.raises(ValueError, match="residual_features"):
        layer(torch.randn(2, 3, 16), torch.randn(2, STATION_FEATURE_DIM))
...
def test_ecbit_sabc_residual_forward_shape() -> None:
    model = ECBITSABC(
        seq_len=24,
        n_vars=5,
        n_time=4,
    ...
    time_enc = torch.randn(4, 24, 4)
    station_features = torch.randn(4, STATION_FEATURE_DIM)
    residual_features = torch.randn(4, 5, RESIDUAL_FEATURE_DIM)

    y = model(x, missing, era5, time_enc, station_features, residual_features)
