...
## Major Revision Diagnostics (2026-05-24)

- Observed-position preservation is enforced by the final copy-back rule: visible AWS inputs are copied into the final imputed output after decoding. A remote CPU audit on retained `followup_mcar_noera5` ECBIT checkpoints found that decoded visible-position outputs have nonzero pre-copy deviation (MAE 0.1608, RMSE 0.2517 over 27 runs), but post-copy visible-position MAE is exactly 0.0. The audit confirms the operational output preserves observed AWS values even though the decoder itself emits a full trajectory.
...
- Remote artifact check: `round2_gated` no longer retains ECBIT `best.pt` checkpoint files, so the gated ECBIT strict subset cannot be re-evaluated from existing artifacts. The fair-baseline iTransformer+ERA5 matrix does retain 27 checkpoints and is the immediately available neural sensitivity target.
- Completed remote non-overlap sensitivity for the 27 retained iTransformer+ERA5 checkpoints. Overall MAE changes from 0.2581 +/- 0.0056 on the full overlapping test-window set to 0.2587 +/- 0.0062 on the deterministic non-overlapping subset. The paired shift is +0.0006 MAE, 95% CI [0.0000, 0.0012], p=0.047. This is practically negligible relative to the main ERA5/no-ERA5 gap (0.088 MAE) and MCAR-to-block transfer gap, so test-window overlap is not driving the ERA5-augmented iTransformer conclusion.

## Post-Review Verification (2026-05-27)

- The newest review concern about station-month calibrated ERA5 is resolved in both statistics and narrative. The direct calibrated baseline is treated as a strong operational reference at 0.270 MAE, and neural ERA5 conditioning is described as a statistically supported but modest improvement rather than a broad replacement for calibrated ERA5.
...
### Figure/Table Audit 2026-05-29
- Audited all 8 figures and 17 tables against their generation scripts and CSV/result artifacts.
- Found and fixed a factual Fig. 3 label issue: the first group included ERA5 direct but was labeled `No-ERA5 baselines`; it is now `Core baselines`.
- Found and fixed table-label clarity issues: station summary elevation is a split median, appendix `aws11` had a trailing-space artifact, and raw-unit q errors needed three decimals to avoid displaying q standard deviation as `0.00`.
