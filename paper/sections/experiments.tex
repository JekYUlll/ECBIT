...
The data pipeline, model implementations, training/evaluation entrypoints, and 315 YAML experiment configurations have been implemented. Remote GPU execution is active on a six-GPU server. The current Round 1 core batch excludes SAITS and BRITS until the main stateless and iTransformer baselines are complete; this allows the interpolation, ERA5-direct, and iTransformer comparisons to be validated before launching the heavier PyPOTS baselines. Preliminary completed runs show a consistent trade-off: ERA5 direct substitution is competitive for long block gaps, while linear interpolation is strongest for short gaps. This supports the central ECBIT hypothesis that ERA5 is useful primarily as a conditional prior during outages rather than as an unconditional replacement for station observations.
